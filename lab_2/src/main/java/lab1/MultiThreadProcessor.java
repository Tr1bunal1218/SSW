...
public class MultiThreadProcessor {
    ...
    static class RandomObjectWriter extends Thread {
        private final int threadNumber; // Порядковый номер потока
        ...
        public void run() {
            ...
            try (FileWriter writer = new FileWriter(filename, true)) {
                for (int i = 0; i < count; i++) {
                    ...
                    printProgress(threadNumber, getId(), (i + 1) * 100 / count); // Используем порядковый номер и ID
                    Thread.sleep(10);
                }
            ...
            executionTime = System.currentTimeMillis() - startTime; // Подсчитываем затраченное время
        }
